...
export default function AdminFeatureFlags({ token, toast }: AdminProps) {
  ...
  return <section className="rounded-xl border border-white/5 bg-[#111] divide-y divide-white/5">{Object.entries(features).map(([key, value]) => <div key={key} className="p-4 flex justify-between items-center"><span className="font-mono text-sm">{key}</span><Switch checked={value} onCheckedChange={(v) => toggle(key, v)} /></div>)}</section>;
}