...
export default function AdminReferrals({ token, toast }: AdminProps) {
  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <div className="overflow-hidden rounded-xl border border-border/70 bg-background"><div className="border-b border-border/70 px-5 py-4 text-sm font-semibold">Top referrers</div><div className="divide-y divide-border/60">{data.referrers.length === 0 ? <p className="p-5 text-sm text-muted-foreground">No referred accounts yet.</p> : data.referrers.map((referrer) => <div key={referrer.id} className="flex items-center justify-between gap-3 px-5 py-3"><div><p className="text-sm font-medium">{referrer.displayName || referrer.username}</p><p className="text-xs text-muted-foreground">@{referrer.username}</p></div><Badge variant="secondary">{referrer.referredUserCount} referred</Badge></div>)}</div></div>
