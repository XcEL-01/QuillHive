...
export default function AdminRevenue({ token, toast }: AdminProps) {
  ...
  return <section className="overflow-hidden rounded-xl border border-border/70 bg-background shadow-sm">
    ...
    {loading ? <p className="p-8 text-sm text-muted-foreground">Loading requests...</p> : boosts.length === 0 ? <p className="p-8 text-center text-sm text-muted-foreground">No boost requests.</p> : <div className="divide-y divide-border/60">{boosts.map((b) => <div key={b.id} className="px-5 py-4 text-sm">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-medium">#{b.id} · {b.plan}</p><Badge variant={b.status === "pending" ? "secondary" : "outline"} className="text-[10px] capitalize">{b.status}</Badge></div><p className="mt-1 truncate text-muted-foreground">{b.postTitle ?? "Post unavailable"} · {b.authorDisplayName ?? b.authorUsername ?? "Creator unavailable"}</p><p className="mt-1 text-xs text-muted-foreground">{b.paidAmountCents != null ? `$${(b.paidAmountCents / 100).toFixed(2)} paid` : "Payment pending"} · {new Date(b.createdAt).toLocaleDateString()}</p></div>
