  ...
  return <>
    ...
    <section className="space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-border/70 bg-background shadow-sm">
        ...
        {loading ? <div className="space-y-3 p-5">{[1, 2, 3, 4].map((row) => <div key={row} className="h-12 animate-pulse rounded bg-muted" />)}</div> : filtered.length === 0 ? <div className="p-12 text-center"><p className="text-sm font-medium">No users found</p><p className="mt-1 text-xs text-muted-foreground">Try a different search.</p></div> : <div className="divide-y divide-border/60">{filtered.map((u) => <div key={u.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_100px_100px_120px] sm:items-center"><Link href={`/profile/${u.username}`} className="flex min-w-0 items-center gap-3 hover:underline"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/10 text-sm font-semibold text-primary">{u.displayName?.[0] ?? u.username?.[0] ?? "?"}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{u.displayName || "Unnamed user"}</p><p className="truncate text-xs text-muted-foreground">@{u.username} · {u.email}</p></div></Link><Badge variant="outline" className="w-fit text-[10px] capitalize">{u.role}</Badge><Badge variant={u.isBanned ? "destructive" : "secondary"} className="w-fit text-[10px]">{u.isBanned ? "Banned" : "Active"}</Badge><Button variant="outline" size="sm" className="w-fit" onClick={() => setNoticeUser(u)}><Send className="mr-1.5 h-3.5 w-3.5" />Send Notice</Button></div>)}</div>}
      </div>
